...
public class MathUtils {
     ...
     * @return
     */
    public static List<List<Long>> groupIdList(List<Long> list) {
        List<List<Long>> idList = new ArrayList<>();
        int size = CustomThreadPoolConfig.GROUP_TEN_THOUSAND_NUM;
        int loopNum = list.size() / size;
        int leftNum = list.size() % size;
        for (int i = 0; i < loopNum; i++) {
            List<Long> ids = new ArrayList<>();
            for (int j = 0; j < size; j++) {
                Long id = list.get(i * size + j);
                ids.add(id);
            }
            idList.add(ids);
        }
        if (leftNum != 0) {
            ...
            for (int i = 0; i < leftNum; i++) {
                Long id = list.get(loopNum * size + i);
                ids.add(id);
            }
            idList.add(ids);
        }
     ...
     * @return
     */
    public static BigDecimal roundingOff(BigDecimal value, int scale) {
        BigDecimal round = null;
        if (value != null) {
            round = value.setScale(scale, BigDecimal.ROUND_HALF_UP);
        }
        return round;
    }
