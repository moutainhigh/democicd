 ...
 */
@Configuration
public class BatchSendMessageTaskExecutor {
    @Bean("batchTaskExecutor")
    public ThreadPoolTaskExecutor taskExecutor() {
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        // 核心线程数：线程池创建时初始化的线程数
        executor.setCorePoolSize(CustomThreadPoolConfig.BATCH_CORE_POOL_SIZE);
        // 最大线程数：线程池最大的线程数，只有在缓冲队列满了之后才会申请超过核心线程数的线程
        executor.setMaxPoolSize(CustomThreadPoolConfig.BATCH_MAX_POOL_SIZE);
        // 缓冲队列：用来缓冲执行任务的队列
        ...
        // 线程池对拒绝任务的处理策略
        executor.setRejectedExecutionHandler(new BatchSendMessageTaskExecutor.CustomRejectedExecutionHandler());
