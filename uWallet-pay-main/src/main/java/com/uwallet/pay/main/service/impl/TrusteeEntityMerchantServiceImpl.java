...
/**
 * @author baixinyue
 * @desctiption 信托类型商户请求docusign
 */
@Slf4j
@Service("trusteeEntityMerchantService")
public class TrusteeEntityMerchantServiceImpl extends BaseServiceImpl implements DocusignRequestService {
...
//        textTabs.put("companyInformation_city", merchantDTO.getCity());
        ...
        boolean trusteeExist = trusteeDTOList != null && trusteeDTOList.size() > 0;
        boolean shareholderExist = shareholderDTOList != null && shareholderDTOList.size() > 0;
        boolean directorExist = directorDTOList != null && directorDTOList.size() > 0;

        if ((merchantDTO.getTrusteeType() != null
                && merchantDTO.getTrusteeType().intValue() == StaticDataEnum.STATUS_0.getCode())
                && trusteeExist) {

            for (int i = 0; i < trusteeDTOList.size(); i++) {
                TrusteeDTO trusteeInfo = trusteeDTOList.get(i);

                String stateName = "";
                if (trusteeInfo.getLicenseState() != null) {
                    Map<String, Object> licenseStateParams = new HashMap<>(16);
                    licenseStateParams.put("code", "merchantState");
                    licenseStateParams.put("value", trusteeInfo.getLicenseState());
                ...
                switch (i) {
                    case 0:
                        textTabs.put("authorisedPerson_firstName_a", trusteeInfo.getFirstName());
                        textTabs.put("authorisedPerson_lastName_a", trusteeInfo.getLastName());
                        textTabs.put("authorisedPerson_DOB_a", trusteeInfo.getBirth());
                        textTabs.put("authorisedPerson_address_a", trusteeInfo.getAddress());
                        if (trusteeInfo.getIdType().intValue() == StaticDataEnum.ID_TYPE_0.getCode()) {
                            textTabs.put("authorisedPerson_passportNumber_a", trusteeInfo.getPassport());
                        } else {
                            textTabs.put("authorisedPerson_driverLicenceNo_a", trusteeInfo.getIdNo());
                            textTabs.put("authorisedPerson_driverLicenseState_a", stateName);
                        }
                        break;
                    case 1:
                        textTabs.put("authorisedPerson_firstName_b", trusteeInfo.getFirstName());
                        textTabs.put("authorisedPerson_lastName_b", trusteeInfo.getLastName());
                        textTabs.put("authorisedPerson_DOB_b", trusteeInfo.getBirth());
                        textTabs.put("authorisedPerson_address_b", trusteeInfo.getAddress());
                        if (trusteeInfo.getIdType().intValue() == StaticDataEnum.ID_TYPE_0.getCode()) {
                            textTabs.put("authorisedPerson_passportNumber_a", trusteeInfo.getPassport());
                        } else {
                            textTabs.put("authorisedPerson_driverLicenceNo_a", trusteeInfo.getIdNo());
                        ...
                        break;
                    case 2:
                        textTabs.put("authorisedPerson_firstName_c", trusteeInfo.getFirstName());
                        textTabs.put("authorisedPerson_lastName_c", trusteeInfo.getLastName());
                        textTabs.put("authorisedPerson_DOB_c", trusteeInfo.getBirth());
                        textTabs.put("authorisedPerson_address_c", trusteeInfo.getAddress());
                        if (trusteeInfo.getIdType().intValue() == StaticDataEnum.ID_TYPE_0.getCode()) {
                            textTabs.put("authorisedPerson_passportNumber_a", trusteeInfo.getPassport());
                        } else {
                            textTabs.put("authorisedPerson_driverLicenceNo_a", trusteeInfo.getIdNo());
                ...
                if (i == trusteeDTOList.size() - 1) {
                    break;
                }
            }

        } else if ((merchantDTO.getTrusteeType() != null
                && merchantDTO.getTrusteeType().intValue() == StaticDataEnum.STATUS_1.getCode())
                && directorExist) {
