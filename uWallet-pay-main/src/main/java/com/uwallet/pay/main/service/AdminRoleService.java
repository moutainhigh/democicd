...
public interface AdminRoleService extends BaseService {
    ...
    * 保存一条 AdminRole 数据
    *
    * @param adminRoleDTO 待保存的数据
    * @param request
    * @throws BizException 保存失败异常
    */
    void saveAdminRole(AdminRoleDTO adminRoleDTO, HttpServletRequest request) throws BizException;

    /**
     * 保存多条 AdminRole 数据
     *
     * @param adminRoleList 待保存的数据列表
     * @throws BizException 保存失败异常
     */
    void saveAdminRoleList(List<AdminRole> adminRoleList) throws BizException;

    /**
     * 修改一条 AdminRole 数据
     *
     * @param id 数据唯一id
     * @param adminRoleDTO 待修改的数据
     * @param request
     * @throws BizException 修改失败异常
     */
    void updateAdminRole(Long id, AdminRoleDTO adminRoleDTO, HttpServletRequest request) throws BizException;

    /**
     * 根据Id部分更新实体 adminRole
     *
     * @param dataMap 需要更新的键值对
     * @param conditionMap where语句后的条件筛选的键值对
     */
    void updateAdminRoleSelective(Map<String, Object> dataMap, Map<String, Object> conditionMap);

    /**
     * 根据id逻辑删除一条 AdminRole
     *
     * @param id 数据唯一id
     * @param request
     * @throws BizException 逻辑删除异常
     */
    void logicDeleteAdminRole(Long id, HttpServletRequest request) throws BizException;

    /**
     * 根据id物理删除一条 AdminRole
     *
     * @param id 数据唯一id
     * @throws BizException 物理删除异常
     */
    void deleteAdminRole(Long id) throws BizException;

    /**
     * 根据id查询一条 AdminRole
     *
     * @param id 数据唯一id
     * @return 查询到的 AdminRole 数据
     */
    AdminRoleDTO findAdminRoleById(Long id);

    /**
     * 根据条件查询得到第一条 adminRole
     *
     * @param params 查询条件
     * @return 符合条件的一个 adminRole
     */
    AdminRoleDTO findOneAdminRole(Map<String, Object> params);
     ...
     * @return 查询结果的数据集合
     */
    List<AdminRoleDTO> find(Map<String, Object> params, Vector<SortingContext> scs, PagingContext pc);
