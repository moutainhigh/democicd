...
public class POIUtils {
     ...
     * @throws Exception
     */
    public static <T> Workbook createExcel(List<T> list, Class<T> clazz, String fileName) throws Exception {
     ...
     * @throws IOException
     */
    public static String writeExcel(Workbook workbook, String url, String fileName) throws IOException {
        // 创建路径
     ...
     * @throws Exception
     */
    public static <T> Workbook createExcelNew(List<T> list, Class<T> clazz, String fileName,List<JSONObject> fristlist) throws Exception {
        fileName = getFileName(fileName);
        // 创建 Workbook 对象(excel 的文档对象)
        Workbook workbook = getWorkbook(fileName);
        // 建立新的 sheet 对象(excel 的表单)
        ...
        for (int i = 0; i < fristlist.size(); i++) {
            ...
            for (String s : fristlist.get(i).keySet()) {
                row1.createCell(0).setCellValue(s);
                row1.createCell(1).setCellValue(fristlist.get(i).getString(s));
            }
        }
...
//            row.createCell(fieldIndex).setCellValue(fields[fieldIndex].getName());
//        }
        String[] heads=new String[]{"Customer Order","Wholesales part in Order Amt.","Order Time","Wholesale Custom Discount","Pay Type","Customer payment","Wholesales part in Actual payment"};
        Field[] fields = clazz.getDeclaredFields();
        for (int fieldIndex = 0; fieldIndex < heads.length; fieldIndex++) {
            row.createCell(fieldIndex).setCellValue(heads[fieldIndex]);
        }
        ...
        if (list != null) {
            ...
            try {
                for (int listIndex = 0; listIndex < list.size(); listIndex++) {
                    T datum = list.get(listIndex);
                    dataRow = sheet.createRow(listIndex + 5);
                    for (int fieldIndex = 0; fieldIndex < fields.length; fieldIndex++) {
                        field = fields[fieldIndex];
                        // 获取单元格
                        Cell cell = dataRow.createCell(fieldIndex);
