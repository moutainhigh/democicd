 ...
 * @modified: baixinyue
 */
public interface AppBannerService extends BaseService {

   /**
    * 保存一条 AppBanner 数据
    *
    * @param appBannerDTO 待保存的数据
    * @param request
    * @throws BizException 保存失败异常
    */
    void saveAppBanner(AppBannerDTO appBannerDTO, HttpServletRequest request) throws BizException;

    /**
     * 保存多条 AppBanner 数据
     *
     * @param appBannerList 待保存的数据列表
     * @param request
     * @throws BizException 保存失败异常
     */
    void saveAppBannerList(List<AppBanner> appBannerList, HttpServletRequest request) throws BizException;

    /**
     * 修改一条 AppBanner 数据
     *
     * @param id 数据唯一id
     * @param appBannerDTO 待修改的数据
     * @param request
     * @throws BizException 修改失败异常
     */
    void updateAppBanner(Long id, AppBannerDTO appBannerDTO, HttpServletRequest request) throws BizException;

    /**
     * 根据Id部分更新实体 appBanner
     *
     * @param dataMap 需要更新的键值对
     * @param conditionMap where语句后的条件筛选的键值对
     */
    void updateAppBannerSelective(Map<String, Object> dataMap, Map<String, Object> conditionMap);

    /**
     * 根据id逻辑删除一条 AppBanner
     *
     * @param id 数据唯一id
     * @param request
     * @throws BizException 逻辑删除异常
     */
    void logicDeleteAppBanner(Long id, HttpServletRequest request) throws BizException;

    /**
     * 根据id物理删除一条 AppBanner
     *
     * @param id 数据唯一id
     * @param request
     * @throws BizException 物理删除异常
     */
    void deleteAppBanner(Long id, HttpServletRequest request) throws BizException;

    /**
     * 根据id查询一条 AppBanner
     *
     * @param id 数据唯一id
     * @return 查询到的 AppBanner 数据
     */
    AppBannerDTO findAppBannerById(Long id);

    /**
     * 根据条件查询得到第一条 appBanner
     *
     * @param params 查询条件
     * @return 符合条件的一个 appBanner
     */
    AppBannerDTO findOneAppBanner(Map<String, Object> params);
     ...
     * @return 查询结果的数据集合
     */
    List<AppBannerDTO> find(Map<String, Object> params, Vector<SortingContext> scs, PagingContext pc);
